...
OUTPUT_DIR = Path("models/sequence_window_transformer_subtechnique")
...
def sample_df(df, target_column, max_rows):
    if max_rows is None or len(df) <= max_rows:
        return df

    return df.groupby(target_column, group_keys=False).apply(
        lambda x: x.sample(
            min(len(x), max(1, max_rows // df[target_column].nunique())),
            random_state=42
        )
...
def main():
    ...
    train_df = sample_df(train_df, TARGET_COLUMN, MAX_TRAIN_ROWS)
    val_df = sample_df(val_df, TARGET_COLUMN, MAX_VAL_ROWS)
    test_df = sample_df(test_df, TARGET_COLUMN, MAX_TEST_ROWS)
    ...
    with open(REPORT_DIR / "sequence_window_transformer_subtechnique_report.txt", "w") as f:
        f.write(report_text)
        f.write("\n\n")
    ...
    pred_df.to_csv(REPORT_DIR / "sequence_window_transformer_subtechnique_predictions.csv", index=False)
