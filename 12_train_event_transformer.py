...
OUTPUT_DIR = Path("models/event_transformer_subtechnique")
...
def sample_df(df, max_rows):
    ...
    return pd.concat(sampled_parts).sample(
        frac=1,
        random_state=RANDOM_STATE
...
def main():
    ...
    val_df = sample_df(val_df, MAX_VAL_ROWS)
    ...
    with open(REPORT_DIR / "event_transformer_subtechnique_report.txt", "w") as f:
        f.write(report_text)
        f.write("\n\n")
    ...
    pred_df.to_csv(REPORT_DIR / "event_transformer_subtechnique_predictions.csv", index=False)

    trainer.save_model(str(OUTPUT_DIR / "best_model"))
    tokenizer.save_pretrained(str(OUTPUT_DIR / "best_model"))

    print("Saved model:", OUTPUT_DIR / "best_model")
    print("Saved report:", REPORT_DIR / "event_transformer_subtechnique_report.txt")
